 ...
 * Note that this means that mini-bots can spawn other mini-bots (if they have the required energy, i.e. at least 100 EU).
 */
case class SpawnWithRole(direction: Heading, name: Option[String] = None, energy: Int, role: String) extends Op {
  require(energy >= 100, "energy must be >= than 100")
  override def toString =
    Util.string("Spawn", "direction" -> direction, "energy" -> energy, "name" -> name, "role" -> role)
}
